...
2. **A fluid is fully described by three numbers:** `min` (relaxed per-cell mass), `max` (per-cell
   compression ceiling), `molarMass` (sort key). "Incompressible" vs "compressible" is just **narrow vs wide
   `min`..`max`** — a per-fluid data setting, NOT an engine branch. Viscosity tunes flow *rate* only.
...
- The `orgeStepWorld` FFI signature is kept ABI-stable. The LUT already carries `molarMass`, `maxMass`,
  `viscosity`, and a min-flow field; the precise mapping of the model's `min` (relaxed density) and `max`
  (compression ceiling) onto LUT fields is finalized in the implementation plan. **The
  `gasFlag`/`airFlag`/`fluidFlag` LUT fields become vestigial to the engine — the engine stops reading them
  for behaviour.** (Kept in the ABI for one transition; the plan removes the reads first, then optionally the
  fields.)
