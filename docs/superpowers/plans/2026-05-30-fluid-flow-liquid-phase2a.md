...
**MUST be surgical — do NOT break obsidian/cobblestone (spec "Vanilla thermal interactions", Decision 2026-05-30 = PRESERVE).** Suppression targets fluid **flow/spread** only. It MUST **whitelist** vanilla's lava↔water contact solidification (lava source + water → obsidian; flowing lava + water → cobblestone; basalt generator) — that result rides the same fluid neighbour-update suppression would otherwise cancel, and silencing it would break **Nether-portal obsidian**, a severe regression. Whatever mechanism is used (event or mixin), the lava↔water → obsidian/cobblestone/basalt path must keep firing. Reproducing these transforms thermally (high-mass quenched lava → obsidian, thin → cobblestone) is a **§7 follow-on, NOT this slice** — see "Not in this plan".
...
- **Reproducing lava↔water transforms thermally** (high-mass quenched lava → obsidian, thin → cobblestone, basalt generator) — a **§7 follow-on**. This slice **PRESERVES** vanilla's contact solidification via surgical suppression (Task 20 whitelist); it does not reproduce it.
- **Superseding vanilla ice/snow freezing** — a **§7 follow-on** (suppress vanilla biome/light water↔ice + snow so §7's simulated-temperature freezing wins). Recorded in the §7 phase-change spec, not here.
