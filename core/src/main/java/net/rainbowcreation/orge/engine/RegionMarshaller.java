...
public final class RegionMarshaller {
    ...
    public static Flat flatten(List<ColumnTask> cols) {
        ...
        return new Flat(n, cx, cz, matIx, mass, tIn, vxIn, vyIn, vzIn);
    }
    ...
    public static List<ColumnResult> slice(char[] matOut, float[] massOut, float[] tOut,
        ...
        for (int c = 0; c < nCols; c++) {
            ...
            out.add(new ColumnResult(mi, ms, tt, vxs, vys, vzs));
        }
        return out;
    }

    /** Back-compat overload: velocity channels zero-filled. Used by tests that don't care about velocity. */
    public static List<ColumnResult> slice(char[] matOut, float[] massOut, float[] tOut, int nCols) {
        ...
        return slice(matOut, massOut, tOut, zeros, zeros, zeros, nCols);
    }
